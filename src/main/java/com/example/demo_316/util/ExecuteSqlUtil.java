...
public class ExecuteSqlUtil<T> {
    ...
    private static final Pattern DANGEROUS_KEYWORDS = Pattern.compile(
        "\\b(ALTER|CREATE|DELETE|DROP|EXEC(UTE)?|INSERT|MERGE|SELECT\\s+\\*\\s+FROM\\s+INFORMATION_SCHEMA|" +
        "UNION(\\s+ALL)?|UPDATE|GRANT|REVOKE|SHUTDOWN|TRUNCATE)\\b", 
    ...
    private void validateSqlStatement(String statement) {
        ...
        // Check for dangerous keywords in non-SELECT statements
        if (!cleanedStatement.trim().toUpperCase().startsWith("SELECT")) {
